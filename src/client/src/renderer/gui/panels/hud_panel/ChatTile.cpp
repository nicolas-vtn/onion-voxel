...
namespace onion::voxel
{
	...
	void ChatTile::Render()
	{
		...
		ColoredBackground::Render(ColoredBackground::CornerOptions{
			.TopLeftCorner = {static_cast<int>(m_Position.x), static_cast<int>(m_Position.y) - textSize.y},
		...
		m_Label.SetPosition({m_Position.x + textSize.x / 2.f, m_Position.y - textSize.y / 2.f});
		m_Label.Render();
	}
	...
	void ChatTile::SetFadingAlpha(float alpha)
	{
		m_FadingAlpha = alpha;
	}
