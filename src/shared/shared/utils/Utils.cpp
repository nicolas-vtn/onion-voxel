...
namespace onion::voxel::Utils
{
	...
	void ReplaceFileAtomic(const std::filesystem::path& targetPath, const std::filesystem::path& tempFilePath)
	{
		...
		if (!MoveFileExW(tempFilePath.wstring().c_str(),
						 targetPath.wstring().c_str(),
						 MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH))
		{
			throw std::runtime_error("MoveFileExW failed replacing file: " + targetPath.string());
		}
#else
		// POSIX: rename is atomic and replaces existing file
		std::error_code ec;
		std::filesystem::rename(tempFilePath, targetPath, ec);

		if (ec)
		{
			throw std::runtime_error("rename failed replacing file: " + ec.message());
		}
#endif
	}
